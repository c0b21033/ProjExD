...
def shutudai(): #問題をランダムに出題する関数
    ...
    questions = ["鮪の読みを答えよ", "魑魅魍魎の読みを答えよ", "亜米利加の読みを答えよ"] #問題のリスト
    answers = [["まぐろ", "マグロ"], ["ちみもうりょう", "チミモウリョウ"], ["あめりか", "アメリカ"]] #答えのリスト
    number = random.randint(0, 2) #問題と答えの番号をランダムに決定し、変数に保存
...
def kaito(number): #ユーザーの解答を受け付け結果を返す関数
    ...
    for i in answers[number]: #ユーザーの解答が答えのリストにあるか調べる
        if ans == i: #正解の場合
            result = "正解！"
            break
    else: #不正解の場合
